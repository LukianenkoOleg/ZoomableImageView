package lukianenko.oleg.zoomableimageview;
...
public class ZoomImageView extends ImageView implements View.OnTouchListener {
    ...
    final static String TAG = "ZoomImageView";
    ...
    private void parseAttributes(Context context, AttributeSet attrs) {
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ZoomImageView, 0, 0);
        try {
            maxZoomLevel = array.getFloat(R.styleable.ZoomImageView_maximumZoomLevel, 3f);
            minZoomLevel = array.getFloat(R.styleable.ZoomImageView_minimumZoomLevel, 1f);
            doubleClickZoomLevel = array.getFloat(R.styleable.ZoomImageView_doubleClickZoomLevel, 2f);
            isMaxZoomLevelRelative = array.getBoolean(R.styleable.ZoomImageView_isMaximumZoomLevelRelativeToView, true);
            isMinZoomLevelRelative = array.getBoolean(R.styleable.ZoomImageView_isMinimumZoomLevelRelativeToView, true);
            adjustToBounds = array.getBoolean(R.styleable.ZoomImageView_adjustToBounds, true);
            doubleClickAdjust = array.getBoolean(R.styleable.ZoomImageView_doubleClickAdjust, true);
            allowExcedLimitsWhenMovingImage = array.getBoolean(R.styleable.ZoomImageView_allowExcedLimitsWhenMovingImage, false);
            blockImageInTheMiddle = array.getBoolean(R.styleable.ZoomImageView_blockImageInTheMiddleWhileIsSmallerThanView, true);
            doubleClickTimeInMillis = array.getInteger(R.styleable.ZoomImageView_doubleClickTimeInMillis, 250);
        } finally {
            array.recycle();
        }
    }
    ...
    public void firstScale() {
        ...
        if (imageWidth < imageHeight) {
            float scaleValue = dm.widthPixels / imageWidth;
            matrix.setScale(scaleValue, scaleValue, 0, 0);
            float postScaleImageHeight = imageHeight * scaleValue;
            float translateValue = (postScaleImageHeight - dm.widthPixels) / 2;
            matrix.postTranslate(0, -translateValue);
        } else {
            float scaleValue = dm.widthPixels / imageHeight;
            matrix.setScale(scaleValue, scaleValue, 0, 0);
            float postScaleImageWidth = imageWidth * scaleValue;
            float translateValue = (postScaleImageWidth - dm.widthPixels) / 2;
